...
def select_folder(root):
    """ユーザーにフォルダを選択させる。引数にはTkinterのルートウィンドウを使用する。"""
    folder_selected = askdirectory(parent=root)  # ダイアログを表示し、選択されたフォルダのパスを取得
    return folder_selected

def get_user_input(root, prompt):
    """ユーザーからの入力を受け取る。引数にはTkinterのルートウィンドウとプロンプトのメッセージを使用する。"""
